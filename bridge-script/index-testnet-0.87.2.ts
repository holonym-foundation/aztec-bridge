...
import { saveTokenToFile, generateSimpleTypescriptFile, loadExistingDeployments, ensureDeploymentEnvironment, DeployedContracts } from './utils/save_contracts.js'
...
const l1Client = createExtendedL1Client(L1_URL.split(','), MNEMONIC, sepolia)
...
async function deployTestERC20(name: string, symbol: string, decimals: number): Promise<EthAddress> {
...
export async function getL2BridgeContractInstance(
  ...
  return await getContractInstanceFromDeployParams(TokenBridgeContract.artifact, {
    salt: salt,
    deployer: deployerAddress,
    constructorArgs: [ownerAztecAddress,l2TokenContract, l1PortalContractAddress],
  })
}

async function mintL1Tokens(l1TokenContract: EthAddress, amount: bigint, logger: Logger, symbol: string) {
  try {
...
async function deployCompleteTokenSetup(
  ...
  logger.info(`Deploying L1 ${tokenConfig.symbol} with decimals ${tokenConfig.decimals} token contract`)
  const l1TokenContract = await deployTestERC20(tokenConfig.l1Name, tokenConfig.l1Symbol, tokenConfig.decimals)
  logger.info(`L1 ${tokenConfig.symbol} token contract deployed at ${l1TokenContract.toString()}`)
  ...
  logger.info(`Fee asset handler for ${tokenConfig.symbol} deployed at ${feeAssetHandler.toString()}`)
  ...
  logger.info(`L1 portal contract for ${tokenConfig.symbol} deployed at ${l1PortalContractAddress.toString()}`)
  ...
  logger.info(`L2 ${tokenConfig.symbol} token contract deployed at ${l2TokenContract.address}`)
  ...
  logger.info(`L2 ${tokenConfig.symbol} bridge contract deployed at ${l2BridgeContract.address}`)
...
async function main() {
  ...
  logger.info(`L1 Chain: ${l1Client.chain?.name || 'Unknown'} (ID: ${l1Client.chain?.id || 'Unknown'})`)
  ...
  try {
    ...
    if (balanceInEth < 0.01) {
      logger.warn('⚠️  Low L1 wallet balance! You may need more ETH for gas fees.')
    }
  } catch (error) {
  ...
  if (existingDeployments) {
    logger.info(`Found existing deployments with ${existingDeployments.tokens.length} tokens`)
    logger.info(`Deployed tokens: ${existingDeployments.tokens.map(t => t.symbol).join(', ')}`)
  }
  ...
  for (const tokenConfig of TOKEN_CONFIGS) {
    ...
    const existingToken = existingDeployments?.tokens.find(t => t.symbol === tokenConfig.symbol)
    if (existingToken) {
    ...
    try {
      ...
      logger.info(`✅ Successfully deployed and saved ${tokenConfig.symbol} token setup`)
    } catch (error) {
  ...
  if (deployedContracts.length > 0) {
    ...
    const l1PortalContractAddress = EthAddress.fromString(firstToken.l1PortalContract)
