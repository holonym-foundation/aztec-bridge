...
import { createPXEClient, EthAddress, generateClaimSecret, PXE, waitForPXE, createLogger, L1TokenManager, L1TokenPortalManager, Fr } from '@aztec/aztec.js';
...
async function main() {
    ...
    const l2TokenContract = await TokenContract.deploy(ownerWallet, ownerAztecAddress, "L2 Token", "L2", 18).send().deployed();
    logger.info(`L2 token contract deployed at ${l2TokenContract.address}`);

    // Deploy L1 token contract & mint tokens

    let l1TokenContract = await deployTestERC20();
    ...
    let l1PortalContractAddress = await deployTokenPortal();
    ...
    let l2BridgeContract = await TokenBridgeContract.deploy(ownerWallet, l2TokenContract.address, l1PortalContractAddress).send().deployed();
    ...
    const l1PortalManager = new L1TokenPortalManager(l1PortalContractAddress, l1TokenContract, l1ContractAddresses.outboxAddress, publicClient, walletClient, logger);
    ...
    await l2BridgeContract.methods.claim_public(ownerAztecAddress, amount, claim.claimSecret, claim.messageLeafIndex).send().wait();
    ...
    const l2ToL1Message = l1PortalManager.getL2ToL1MessageLeaf(withdrawAmount, EthAddress.fromString(ownerEthAddress), l2BridgeContract.address, EthAddress.ZERO);
    const l2TxReceipt = await l2BridgeContract.methods.exit_to_l1_public(EthAddress.fromString(ownerEthAddress), withdrawAmount, EthAddress.ZERO, nonce).send().wait();
