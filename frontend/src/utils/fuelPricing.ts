...
export function buildSwapCandidates(inputToken: `0x${string}`): CandidateRoute[] {
  ...
  if (inputToken.toLowerCase() === weth.toLowerCase()) {
    ...
    candidates.push({
      label: 'direct-weth',
      poolKeys: [buildPoolKey(feePoolBase, aztec, FEE_POOL_FEE, FEE_POOL_TICK_SPACING, FEE_POOL_HOOKS)],
  ...
  for (const tier of intermediateTiers) {
    candidates.push({
      ...
      poolKeys: [
        ...
        buildPoolKey(feePoolBase, aztec, FEE_POOL_FEE, FEE_POOL_TICK_SPACING, FEE_POOL_HOOKS),
      ],
